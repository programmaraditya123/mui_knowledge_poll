...
const Fla = () => {
  ...
  const [searchtitle, setSearchTitle] = useState(title === "" || title === undefined ? "Introduction to Formal Languages and Automata" : title);
...
const fla_topics = [
  ...
  const getContent = async () => {
    ...
    try {
      const data = await axios.get(`${BASE_URL}/app/getcont/getflacontent`, { params: { title: searchtitle } });
    ...
    <Suspense fallback={<div>Loading ...</div>}>
      ...
      <div className='disp-cont'>
        ...
        <div className='disp-cont-2'>
          ...
          <button className='btn-18' onClick={() => routerchange(searchtitle,27)}>Write Content</button> : <button className='btn-18' onClick={() => routerchange1(searchtitle,cont,27)}>Modify Content</button>
          }
