...
const Dbms = () => {
  ...
  const [searchtitle, setSearchTitle] = useState(title === "" || title === undefined ? "Introduction to DBMS" : title);
...
const dbms_topics = [
  ...
  const getContent = async () => {
    ...
    try {
      const data = await axios.get(`${BASE_URL}/app/getcont/getdbmscontent`, { params: { title: searchtitle } });
    ...
    <Suspense fallback={<div>Loading ...</div>}>
      ...
      <div className='disp-cont'>
        ...
        <div className='disp-cont-2'>
          ...
          <button className='btn-18' onClick={() => routerchange(searchtitle,25)}>Write Content</button> : <button className='btn-18' onClick={() => routerchange1(searchtitle,cont,25)}>Modify Content</button>
          }
