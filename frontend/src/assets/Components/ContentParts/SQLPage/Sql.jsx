  ...
  return (
    ...
    <Suspense fallback={<div>Loading ...</div>}>
      ...
      <div className='disp-cont'>
          ...
          <div className='disp-cont-items'>
            <ul>
              ...
              {sql_topics.map((topics, index) => (
               <li key={index}><Link to={`/sql/${topics.replace(/\s+/g,'-')}`}>{topics}</Link></li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <CreatorViews formattedTopic={formattedTopic} tit="sql" tagn={31}/>

        <NextPrevTopic topics={sql_topics} currentTopic={formattedTopic} basePath="/sql"/>
        </div>
