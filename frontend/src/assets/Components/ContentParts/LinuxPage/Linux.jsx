...
const Linux = () => {
  ...
  const [searchtitle, setSearchTitle] = useState(title === "" || title === undefined ? "Introduction to Linux" : title);
...
const linux_topics = [
  ...
  const getContent = async () => {
    ...
    try {
      const data = await axios.get(`${BASE_URL}/app/getcont/getlinuxcontent`, { params: { title: searchtitle } });
    ...
    <Suspense fallback={<div>Loading ...</div>}>
      ...
      <div className='disp-cont'>
        ...
        <div className='disp-cont-2'>
          ...
          <button className='btn-18' onClick={() => routerchange(searchtitle,43)}>Write Content</button> : <button className='btn-18' onClick={() => routerchange1(searchtitle,cont,43)}>Modify Content</button>
          }
